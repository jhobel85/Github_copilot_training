...
def _run_uvicorn_request():
    ...
    try:
        ...
        while time.monotonic() < deadline:
            try:
                request = Request(
                    ...
                    headers={"X-Request-ID": "inventory-subprocess"},
                )
        ...
        assert response.headers["X-Request-ID"] == "inventory-subprocess"
    finally:
...
def test_supplied_request_id_is_returned_in_structured_log_without_secrets(caplog):
    with caplog.at_level(logging.INFO):
        response = client.post(
            "/inventory",
            headers={"X-Request-ID": "inventory-trace", "X-API-Key": "api-key-secret"},
            json={"productId": "body-secret", "warehouse": "WH-EAST", "quantity": 5},
        )

    record = next(record for record in _request_log_records(caplog) if record.request_id == "inventory-trace")
    payload = json.loads(_structured_handler().formatter.format(record))

    assert response.headers["X-Request-ID"] == "inventory-trace"
    assert payload["request_id"] == "inventory-trace"
    assert payload["service"] == "inventory"
    assert payload["method"] == "POST"
    assert payload["path"] == "/inventory"
...
def test_concurrent_requests_keep_request_ids_isolated(caplog):
    request_ids = [f"inventory-concurrent-{index}" for index in range(8)]
    ...
    logged_ids = {
        ...
        if record.request_id.startswith("inventory-concurrent-")
    }
...
def test_real_uvicorn_stream_contains_only_structured_logs_without_secrets():
    ...
    assert payloads
    assert all(payload["service"] == "inventory" for payload in payloads)
    assert all(payload["request_id"] in {None, "inventory-subprocess"} for payload in payloads)
    assert any(payload["logger"] == "uvicorn.error" for payload in payloads)
    assert access_payloads == [
        {
            **access_payloads[0],
            "message": "HTTP request",
            "request_id": "inventory-subprocess",
            "method": "GET",
            "path": "/health",
            "status_code": 200,
        }
...
def test_unhandled_exception_returns_and_logs_request_id(caplog):
    ...
    with caplog.at_level(logging.ERROR):
        response = failing_client.get("/explode", headers={"X-Request-ID": "inventory-failure"})

    assert response.status_code == 500
    assert response.text == "Internal Server Error"
    assert response.headers["X-Request-ID"] == "inventory-failure"
    ...
    assert payload["request_id"] == "inventory-failure"
    assert payload["status_code"] == 500
    assert payload["exception_type"] == "RuntimeError"
    assert "private inventory failure" not in response.text
