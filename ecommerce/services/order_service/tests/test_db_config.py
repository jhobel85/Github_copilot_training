"""Engine configuration guarantees for the Order Service test deployment."""
...
def test_file_backed_test_engine_gets_busy_timeout() -> None:
    # The order test deployment is file-backed so concurrency tests use independent
    ...
    assert busy_timeout_for("sqlite:////data/order_service.db") == 30.0
...
def test_busy_timeout_classification() -> None:
    assert busy_timeout_for("sqlite:////data/order_service.db") == 30.0
    assert busy_timeout_for("sqlite:///./order_service.db") == 30.0
    assert busy_timeout_for("sqlite:///:memory:") is None
    assert busy_timeout_for("postgresql+psycopg://u:p@db/orders") is None
